    ...
    const fetchClasses = async () => {
      ...
      try {
        ...
        const classList = response.items.map((item: any) => {
          // 1. Handle the single Date and Time field from Contentful
          let formattedDate = '';
          let formattedTime = '';
          ...
          return {
            ...
            org: item.fields.organizationInCharge || '',
            modality: item.fields.classModality || 'Face-to-Face',
            date: formattedDate,
            time: formattedTime,
            venue: item.fields.venue || '',
            slots: item.fields.numberOfSlots || 0,
            subtheme: item.fields.subtheme || '',
            // Connect the actual poster image ID
            image: item.fields.posterPublishingMaterial?.fields?.file?.url
              ? `https:${item.fields.posterPublishingMaterial.fields.file.url}`
              : 'https://picsum.photos/seed/leap/400/250',
            // Pull the new org logo you added!
            orgLogo: item.fields.organizationInChargeLogo?.fields?.file?.url
              ? `https:${item.fields.organizationInChargeLogo.fields.file.url}`
              : null,
            googleFormUrl: item.fields.registrationLink || '',
            // Keeping this fallback until you decide what to do with descriptions
